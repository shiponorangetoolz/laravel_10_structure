...
{
    public static function applClasses($config = 'custom'): array
    {
        // Demo
    ...
    public static function updatePageConfigAdmin($pageConfigs)
    {
        $demo = 'custom';
        $fullURL = request()->fullurl();
        if (App()->environment() === 'production') {
        ...
        if (isset($pageConfigs)) {
            if (count($pageConfigs) > 0) {
                foreach ($pageConfigs as $config => $val) {
                    Config::set('custom-admin.' . $demo . '.' . $config, $val);
                }
            }
        }
    }
